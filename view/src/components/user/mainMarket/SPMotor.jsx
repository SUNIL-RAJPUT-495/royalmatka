...
export const generateSPMotorPanas = (digitsStr) => {
  const clean = digitsStr.replace(/\D/g, '');
  const digits = Array.from(new Set(clean.split('')))
    .map(Number)
    .sort((a, b) => a - b);

  if (digits.length < 3) {
    return { panas: [], subtext: clean ? `Digits: ${digits.join(', ')} — 0 panas found` : '' };
  }
  ...
  const subtext = `Digits: ${digits.join(', ')} — ${panas.length} panas found`;
  return { panas, subtext };
};

export const SPMotor = ({ 
  session, 
  setSession, 
  setBidsList, 
  themeColor,
  ...
  const { panas, subtext } = generateSPMotorPanas(digitsInput);

  const handleAddMoreSPMotor = () => {
    setErrorBannerMsg('');
    const cleanDigits = digitsInput.replace(/\D/g, '');

    if (cleanDigits.length < 3) {
      const msg = 'Pana requires 3 digits! Please enter at least 3 unique digits for SP Motor.';
      setErrorBannerMsg(msg);
      toast.error(msg);
      return;
    }
    if (panas.length === 0) {
      const msg = 'No valid panas generated! Enter unique digits.';
      setErrorBannerMsg(msg);
      toast.error(msg);
      return;
    }
    ...
    setPoints('');
    setErrorBannerMsg('');
    toast.success(`${newBids.length} SP Motor bids added to list! ➕`);
  };
