...
export const TriplePanaBulk = ({ 
  ...
  const [selectedPanas, setSelectedPanas] = useState([]); // Selected Triple Panas, e.g. ['000', '222', '333']
  const [points, setPoints] = useState('');

  const triplePanasList = ['000', '111', '222', '333', '444', '555', '666', '777', '888', '999'];

  // Toggle selection of a Triple Pana
  ...
  // Add bulk bids for all selected Triple Panas
    ...
    <div className="bg-white rounded-xl p-4 border border-gray-200/80 shadow-3xs space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-5 gap-2">
          {triplePanasList.map((p) => {
            const isSelected = selectedPanas.includes(p);
            return (
              <button
                key={p}
                type="button"
                onClick={() => togglePana(p)}
                style={{ backgroundColor: isSelected ? themeColor : '#f0f2f5' }}
                className={`h-11 rounded-xl font-bold text-xs transition-all active:scale-95 cursor-pointer shadow-3xs flex items-center justify-center ${
